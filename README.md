...
## Solution :
We are using AWS to save on storage costs. We created a Smart Lambda function that looks at our snapshots and EC2 instances. If Lambda finds a snapshot that is not connected to any active EC2 instances, it will delete that snapshot to save on costs. This helps us reduce AWS costs.

## Note :
There are many similar issues like this. For example, we can attach an Elastic IP to an EC2 instance but forget to delete the Elastic IP after the EC2 instance is terminated. In this case, the Elastic IP continues to incur costs.
